...
const App = () => {
  ...
  return (
    <>
    <div className='mx-auto max-w-md m-5 p-4 rounded-2xl'>
          <h1>Password Generator</h1>
          <div className='flex gap-2'>
            <input type="text" value={password} readOnly
            ref={passwordRef} className='border rounded-lg w-full' />
            <button onClick={copypasswordToClipboard} 
            className='p-1 bg-green-400 rounded-lg'>Copy</button>
          </div>
          <div className='flex gap-2 mt-2'>
            <input type="range" min={1} max={30} onChange={(e)=>setLength(e.target.value)} /> 
            ...
            <input type="checkbox" id='string' onClick={()=>setString(prev=>!prev)} />
          </div>
